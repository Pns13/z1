...
def login_lagi334():
	try:
		...
		print(f'  {u}[{h}•{u}]{h} LOGIN BERHASIL.........{k} ');time.sleep(1)
		exit()
	except Exception as e:
...
def menu(my_name,my_id):
	...
	print('\tSelamat Datang %s ANAK HARAM'%(my_name)) 
	popen_xd(f'{u}ID  : '+str(my_id))
	popen_xd(f'{h}IP  : {ip}')
	print('\t           • PILIH PENGEN NGAPAIN LO • ')
...
def dump_massal():
	try:
		...
		exit()
		print('Pastikan Gunakan ID publik...') 
	try:
...
def setting():
	...
	if hu in ['1','01']:
		...
		exit()
	print('PILIH METODE LOGIN MU') 
	print('[1] Mobile(Sangat Disarankan)\n[2] Mbasic(Disarankan)\n[3] Touch\n[4] Mtouch [bold purple]')
	print('')
	hc = input('>> Pilih : ')
	if hc in ['1','01']:
